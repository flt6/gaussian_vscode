...
exports.GaussianCompletionProvider = void 0;
const vscode = require("vscode");
class GaussianCompletionProvider {
    constructor() {
        // Gaussian关键字数据
        this.keywords = {
            // 计算方法
    ...
    getRouteKeywordsAndMethods(lineText) {
        const completions = [];
        // 检查当前行是否已经包含计算方法
        const hasMethod = this.keywords.methods.some(method => lineText.toLowerCase().includes(method.toLowerCase()));
        if (hasMethod) {
            // 如果已经有方法，只显示关键字
            // 添加任务类型
    ...
    getParameterCompletions(linePrefix) {
        const completions = [];
    ...
    getLink0Completions(position) {
        ...
        this.keywords.link0Commands.forEach(command => {
            ...
            // 设置替换范围，避免重复%
            item.range = new vscode.Range(new vscode.Position(position.line, 0), new vscode.Position(position.line, position.character));
            // 为某些命令添加默认值
            switch (command) {
                case '%mem':
...
exports.GaussianCompletionProvider = GaussianCompletionProvider;
