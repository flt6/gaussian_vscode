...
export function activate(context: vscode.ExtensionContext) {
    ...
    // 注册运行输入文件命令
    const runInputCommand = vscode.commands.registerCommand('gaussian.runInput', (uri?: vscode.Uri) => {
        // 如果没有传入URI，使用当前活动编辑器的文件
        if (!uri) {
            const activeEditor = vscode.window.activeTextEditor;
            if (activeEditor) {
...
async function runGaussianInput(uri: vscode.Uri) {
...
async function waitForOutputFile(outputFilePath: string, maxWaitTime: number = 30000) {
